...
func (h *IntegrationEventHandler) HandleMessage(ctx context.Context, topic string, payload []byte) error {
	...
	switch eventType := effectiveEventType(event); eventType {
	case "energy.consumption.recorded":
		return h.handleEnergyEvent(ctx, event)
	case "analytics.anomaly.detected", "analytics.recommendation.generated":
		return h.handleAnalyticsEvent(ctx, event)
	default:
		h.logDiscardedEvent(event, "unsupported event type")
		return nil
	}
}
...
func (h *IntegrationEventHandler) handleEnergyEvent(ctx context.Context, event integrationEvent) error {
	...
	if !matchesAnyEventType(eventType, "energy.consumption.recorded") {
...
func (h *IntegrationEventHandler) handleAnalyticsEvent(ctx context.Context, event integrationEvent) error {
	eventType := effectiveEventType(event)
	if !matchesAnyEventType(eventType, "analytics.anomaly.detected", "analytics.recommendation.generated") {
	...
	h.logger.Printf(
		"analytics alert event accepted topic=%s eventType=%s user_id=%s device_id=%s alert_type=%s",
		event.Topic,
		eventType,
		userID,
		deviceID,
	...
	h.logger.Printf(
		"analytics alert creation attempt topic=%s eventType=%s user_id=%s device_id=%s alert_type=%s",
		event.Topic,
		eventType,
		userID,
		deviceID,
	...
	if _, err := h.alertService.CreateAlertAndNotify(ctx, cmd); err != nil {
		h.logger.Printf("analytics alert creation error topic=%s eventType=%s: %v", event.Topic, event.EventType, err)
		return nil
	}

	h.logger.Printf(
		"analytics alert persisted topic=%s eventType=%s user_id=%s device_id=%s alert_type=%s",
		event.Topic,
		eventType,
		userID,
		deviceID,
...
func parseIntegrationEvent(topic string, payload []byte) (integrationEvent, error) {
	...
	eventType := firstNonEmptyString(values, []string{"eventType", "event_type"}, "")
	if eventType == "" {
		return integrationEvent{}, fmt.Errorf("invalid integration event on topic %s: missing eventType", strings.TrimSpace(topic))
	}
